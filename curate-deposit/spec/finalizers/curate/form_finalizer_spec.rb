...
module Curate
  describe FormFinalizer do
    ...
    let(:config) do
      {
        ...
        on_load_from_persistence: { load_attributes: lambda {|form| form.title = persisted_title } },
        identity_minter: minter
      }
    end
    let(:title_publisher) { double }
    let(:persisted_title) { 'Persisted Title'}
    ...
    context 'instance of base_class' do
      ...
      context 'initialized with persisted identifier' do
        subject { base_class.new(controller, minted_identifier) }
        its(:title) { should eq persisted_title }
      end

      context 'initialized without a persisted identifier' do
